...
function StatTile({ label, value, accent }) {
  return (
    <div
      style={{
        flex: 1, border: "1px solid var(--border-accent)", borderRadius: "var(--radius-md)",
...
export default function Home() {
  ...
  const unrunMeetings = summary ? summary.catchup.meetings.filter((m) => !m.has_run).length : 0;

  // Compute a single "top priority" across all three capabilities, for the hero banner.
  let topPriority = null;
  if (summary) {
    if (summary.planner.has_plan && summary.planner.top_priority) {
      topPriority = { text: summary.planner.top_priority, source: "Daily Plan", cta: "View Plan", to: "/planner" };
    } else if (unrunMeetings > 0) {
      const m = summary.catchup.meetings.find((m) => !m.has_run);
      ...
      {summary && (
        <>
          ...
          <div style={{ display: "flex", gap: 14, marginBottom: 20 }}>
            ...
            <StatTile label="Plan items today" value={summary.planner.plan_item_count} accent="var(--success)" />
            <StatTile label="Readiness milestone" value="E3 rollout" accent="var(--danger)" />
          </div>
          ...
          {topPriority && (
            <div
              style={{
                ...
                padding: "18px 22px", borderRadius: "var(--radius-lg)", marginBottom: 24,
                background: "linear-gradient(90deg, var(--accent-primary-tint), var(--surface-canvas))",
            ...
            >
              ...
              <button
                onClick={() => navigate(topPriority.to)}
                style={{
                  flexShrink: 0, background: "var(--accent-primary)", color: "#fff", border: "none",
                  borderRadius: "var(--radius-md)", padding: "10px 18px", fontSize: 13.5, fontWeight: 600, cursor: "pointer",
                }}
              >
                {topPriority.cta} →
              </button>
